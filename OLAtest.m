...
[Waveformlength, ChannelNum]=size( Waveform );  %obatain signal length and channel number
FrameLength=length( Hammingwindow );        %obtain FrameLength

FrameNum=ceil(( Waveformlength - FrameLength )/ Windowshift )+1;  %calculate Frame number
Zeronumber=( FrameNum - 1 )* Windowshift - ( Waveformlength - FrameLength ); %calculate number of zeros to be padded at the last window
...
for m=1:ChannelNum
    ...
    ProcessWaveform(:,m)=Synthesiser( STFTsignal , ChangedShift );  %synthesis STFT signal
end
